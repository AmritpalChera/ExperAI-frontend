...
export default function GroupCard({ group, setActiveGroup, index }: any) {
  ...
  return (
    <div key={group.groupId} onClick={()=>setActiveGroup(group)} className="shadow flex bg-gray-50  max-w-3xl gap-4 border p-4 rounded-lg hover:shadow-lg cursor-pointer">
        ...
        <div className="flex gap-1 w-full max-w-3xl overflow-hidden flex-wrap">
          {group?.npcId?.tags?.map((tag: string) => <div key={tag} className="px-2 mb-2 rounded-full text-sm text-secondary bg-green-100">{tag}</div>)}
        </div>
